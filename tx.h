/*------------------------------------------------------+----------------------
// МикроМир07          Texts - Тексты (header)          | (c) Epi MG, 2007-2008
//------------------------------------------------------+--------------------*/
#ifndef TX_H_INCLUDED        /* Old tx.h (c) Attic 1989, (c) EpiMG 1998,2001 */
#define TX_H_INCLUDED

txt *TxNew(BOOL qundo); void TxInit();
 ...
 */
small tctoaf (tchar *orig, int len, char  *dest_buf); /* returns destination */
small aftotc (char  *orig, int len, tchar *dest_buf); /* size, in char/bytes */
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
